setwd('D:/Data/0_lixia/0)_8e_HTseq_all/0_Paper_Figure_Data_source_2024/0_d10mu2/080_cas13_gRNA_overlap_PLOT/genome_pos')
...
cols = c("Day7" = "#3E4059", "Day14" = "#D18067")  # Green for fc1, Pink for fc2
sample = 'NES_NLS'
#sample = 'K562_NES_K562_NLS'
xlim = c(-0.2, 1.25) 
xb=seq(0, 1.25, by = 0.25)
# Load the data
data1 = read.table(str_c(sample, '.sup5_HEK293T.genome_pos.base_count'), header = TRUE, sep = '\t')
data1$fc1=0-data1$fc1
data1$fc2=0-data1$fc2
head(data1)
# Reshape the data into long format
data_long <- reshape(data1, 
                     varying = c('number', 'number.1'), 
                     v.names = 'value', 
                     timevar = 'type', 
                     times = c('number', 'number.1'), 
                     direction = 'long')

# Modify 'type' column to have values "A" and "I"
data_long$type <- ifelse(data_long$type == "number", "Cyt", "Nuc")

# Filter out rows where 'value' is greater than 10
data_long <- data_long %>% filter(value <= 9)
...
data_long_fc <- data_long %>%
  ...
  mutate(
    ...
    shape = ifelse(type == "Cyt", 16, 17),  # Shape for A (solid circle) and I (solid triangle)
...
significance_results <- data_long_fc %>%
  ...
  summarise(
    ...
    significance = ifelse(mw_test_result < 0.05, ifelse(mw_test_result < 0.001,"**","*"), NA)
  )
...
ggplot(df_summary_fc, aes(x = value, y = mean_value, color = fc_label, 
  ...
  scale_shape_manual(values = c("Cyt" = 22, "Nuc" = 25)) +  # Shapes for A and I types
  ...
  geom_text(data = df_summary_fc %>%
              filter(type == "Cyt" & !is.na(significance) & fc_type == "fc1"),  # Filter to only include type "A" and significance
            aes(label = significance,
                y = ifelse(value < 9, ifelse(value <7,ei_upper - 0.1,ifelse(value==8,ei_upper - 0.17,ei_upper - 0.15)), ei_upper - 0.2)),
            color = "#3E4059", size = 10
  ) +  # Adjust size of the star markers
  geom_text(data = df_summary_fc %>%
              filter(type == "Nuc" & !is.na(significance) & fc_type == "fc2"),  # Filter to only include type "A" and significance
            aes(label = significance,
                y = ei_upper + 0.05),
            color = "#D18067", size = 10
  ) +  # Adjust size of the star markers
  # Add the 'n' count for 'A' type only
  geom_text(data = df_summary_fc %>% filter(type == "Cyt" & fc_type == "fc1"),  # Filter for type A only
            aes(label = n, 
                y = ifelse(value < 9 , ifelse(value <8,ifelse(value==7,ei_upper - 0.17,ei_upper - 0.15),ei_upper - 0.2),ei_upper - 0.25)),  # Adjust y position for count text of A
            size = 3, color = "#3E4059",face='bold') +  # Set color for A counts (blue)
  # Add the 'n' count for 'I' type only
  geom_text(data = df_summary_fc %>% filter(type == "Nuc" & fc_type == "fc2"),  # Filter for type I only
            aes(label = n, 
                y = ifelse(value==9,ei_upper+0.05,ei_upper + 0.15)),  # Adjust y position for count text of I
            size = 3, color = "#D18067",face='bold')   # Set color for I counts (red)
